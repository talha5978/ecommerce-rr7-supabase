...
export function getSanitizedMetaDetailsForForm({ formData, normalizedValues, entity, hasChanges }: FormProps) {
    ...
    const initialKeywords = entity.meta_details!.meta_keywords!.split(",")
        .map((kw: string) => kw.trim())
        .filter((kw: string) => kw !== "");
