...
import type { CouponFormValues } from "@ecom/shared/schemas/coupons.schema";
import type { BuyXGetYGroupOpts, TypesToSelect } from "@ecom/shared/types/coupons-comp";
...
export const BuyXGetYCard = ({ control, disabled }: BuyXGetYCardProps) => {
	...
	const SelectionArea = ({ field, group }: SelectionFuncProps) => {
		if (group === "buy") {
			...
			const data = responses[selectedBuyType as keyof typeof responses];
			return data
				? getSelectTypes({
			...
			const data = responses[selectedGetType as keyof typeof responses];
			return data
				? getSelectTypes({
									...
									<FormControl>
										<div className="*:w-full">
											...
											>
												...
												<SelectContent>
													{typesToSelect.map((type: TypesToSelect) => (
