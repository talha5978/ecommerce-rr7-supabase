...
const AddressSettings = memo(function AddressSettingsFunc({
	loaderData,
}: {
	loaderData: {
	...
	const onSubmit = (data: UpdateStoreAddressData) => {
		...
		fetcher.submit(formData, {
			method: "PATCH",
			action: `/settings/store-details/${store_id}/address/update`,
		});
	};
		...
		<form onSubmit={handleSubmit(onSubmit)}>
			<Card>
				...
				<CardContent className="space-y-4">
					<div>
						<Suspense fallback={<Skeleton className="w-full h-[var(--map-height)]" />}>
							<Controller
								name="address"
								control={control}
								render={({ field }) => (
